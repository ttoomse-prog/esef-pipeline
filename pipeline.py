...
def fetch_new_uk_filings(since_date: str | None, limit: int) -> list[dict]:
    """
    Fetch recent UK filings not yet in our state.
    since_date: ISO date string e.g. '2025-01-01'
    """
    params = {
        "filter[country]": "GB",
        "sort": "-added_time",
        "page[size]": min(limit * 3, 50),   # fetch more than needed, filter after
    }
    if since_date:
        params["filter[added_time][gte]"] = since_date
...
def get_zip_url(filing: dict) -> str | None:
    ...
    try:
        # filings.xbrl.org returns download links in relationships or attributes
        attrs = filing.get("attributes", {})
        # Try direct package_url
        if attrs.get("package_url"):
            return attrs["package_url"]
        # Try links
...
def run(limit: int = MAX_NEW_PER_RUN, dry_run: bool = False, watchlist_only: bool = False):
    ...
    if not watchlist_only:
        ...
        since = state.get("last_run", {}).get("date") if isinstance(state.get("last_run"), dict) \
            else state.get("last_run")

        filings = fetch_new_uk_filings(since_date=since, limit=remaining)
        time.sleep(REQUEST_DELAY)
